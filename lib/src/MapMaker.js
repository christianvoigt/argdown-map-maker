    ...
    value: function makeMap(data) {
      ...
      try {
        for (var _iterator = statementKeys[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          if ((!this.settings.excludeDisconnected || isConnected) && selectionTest) {
            ...
            if (this.settings.statementLabelMode != "text" || _.isEmpty(_node2.labelText)) {
              if (this.settings.statementLabelMode == 'title' || !statementKey.startsWith('Untitled')) {
                _node2.labelTitle = statementKey;
              }
            }
            if (this.settings.addTags && _equivalenceClass.sortedTags) {
      ...
      try {
        for (var _iterator2 = argumentKeys[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          ...
          if (this.settings.argumentLabelMode != 'description' || _.isEmpty(_node3.labelText)) {
            if (!_argument.title.startsWith('Untitled') || this.settings.argumentLabelMode == 'title') {
              _node3.labelTitle = _argument.title;
            }
          }
          if (this.settings.addTags && _argument.sortedTags) {
      ...
      try {
        for (var _iterator3 = relationsForMap[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
          ...
          if (!fromNode) {
            //c): no node representing the source, so look for all arguments that use the source as conclusion
            var _roles = statementRoles[_relation4.from.title];
            fromStatement = data.statements[_relation4.from.title];
            if (_roles && _roles.conclusionIn) {
              froms.push.apply(froms, _roles.conclusionIn);
            }
          } else {
            //a) or b): push either the argument node or the statement node to the sources list
            froms.push(fromNode);
          }
          ...
          if (!toNode) {
            //c): no node representing the target, so look for all arguments that use the target as premise
            var _roles2 = statementRoles[_relation4.to.title];
            toStatement = data.statements[_relation4.to.title];
            if (_roles2 && _roles2.premiseIn) {
              tos.push.apply(tos, _roles2.premiseIn);
            }
          } else {
            //a) or b): push either the argument node or the statement node to the targets list
            tos.push(toNode);
          }
          ...
          if (_relation4.type == "contradictory") {
            ...
            var toRoles = statementRoles[_relation4.to.title];
            if (fromRoles && fromRoles.premiseIn) {
              ...
              try {
                for (var _iterator16 = fromRoles.premiseIn[Symbol.iterator](), _step16; !(_iteratorNormalCompletion16 = (_step16 = _iterator16.next()).done); _iteratorNormalCompletion16 = true) {
                  ...
                  if (toNode && toNode.type == "statement") {
                    // toNode is statementNode
                  ...
                  } else if (toRoles && toRoles.conclusionIn) {
                    // there are arguments that have the to statement as conclusion
                    var _iteratorNormalCompletion17 = true;
                    var _didIteratorError17 = false;
                    var _iteratorError17 = undefined;

                    try {
                      for (var _iterator17 = toRoles.conclusionIn[Symbol.iterator](), _step17; !(_iteratorNormalCompletion17 = (_step17 = _iterator17.next()).done); _iteratorNormalCompletion17 = true) {
                        var toArgument = _step17.value;
